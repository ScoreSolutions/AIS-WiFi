...
Public Class frmInputPassport
    ...
    Private Sub frmInputPassport_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing

        pbPassportPreview.Image = Nothing
        pbPassportDisplay.Image = Nothing
        webcam.Stop()
    End Sub

    Private Sub frmInputPassport_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            Me.Location = New Point(0, 0)
        

            'PictureBox1.BackgroundImage=Image.
            pbPassportInfo.Parent = pbPassportPreview
            pbPassportInfo.BackColor = Color.Transparent

            SplitContainer1.Parent = Me
            SplitContainer1.BackColor = Color.Transparent

            webcam = New WebCam()
            webcam.InitializeWebCam(pbPassportPreview)
            webcam.Start()
            webcam.CaptureHeight(pbPassportPreview.Height)
            webcam.CaptureWidth(pbPassportPreview.Width)
            'lblImagePath.Text = ""

            Path = Application.StartupPath & "\TempPassportCaptureImage\"
...
#Region "OCR"
    Private Sub SetOCRObject()
        ...
        gr = pbPassportDisplay.CreateGraphics()
    ...
    Private Sub SetOCRImage()
        ...
        Else
            ...
            Else
                ...
                Else 'load image from PictureBox
                    ...
                    pbPassportDisplay.Image = bmp
                    pbPassportDisplay.Image.Save(ms, Drawing.Imaging.ImageFormat.Png)
                    data = ms.ToArray()
        ...
        If res > TNSOCR.ERROR_FIRST Then
            ...
            Else
                ...
                showFormError("Img_LoadFile : " & res)
            End If
            Exit Sub
        End If
    ...
    Public Sub DoImageLoaded()
        ...
        If res > TNSOCR.ERROR_FIRST Then
            showFormError("Img_GetPageCount : " & res)
            Exit Sub
        End If
        If res > 1 Then
    ...
    Private Sub ShowImage()
        ...
        pbPassportDisplay.Image = bmp2
        pbPassportDisplay.BringToFront()
        GC.Collect()
    End Sub
...
#End Region
    ...
    Private Sub btnPassportCapture_Click(sender As Object, e As EventArgs) Handles btnPassportCapture.Click
        If pbPassportPreview.Image Is Nothing Then
            ShowDialogBox("No Image Capture", "No Image", False)
            Exit Sub
        End If

        webcam.Stop()

        CardFileName = "Passport.jpg"
        ...
        Try
            pbPassportPreview.Image.Save(PathFile, System.Drawing.Imaging.ImageFormat.Jpeg)
            ...
            If IO.File.Exists(PathFile) = True Then
                ...
                pbPassportDisplay.Image = imag
                pbPassportDisplay.BackgroundImageLayout = ImageLayout.Zoom
                pbPassportDisplay.SendToBack()
    ...
    Private Sub Recogizion()
        If Not IsImgLoaded() Then
            showFormError("Image not loaded!")
            Exit Sub
        End If
        ...
        Else
            ...
            If res > TNSOCR.ERROR_FIRST Then
                showFormError("Ocr_OcrImg(1) : " & res)
                Exit Sub
            End If
        End If
        ...
        If res > TNSOCR.ERROR_FIRST Then
            If res = TNSOCR.ERROR_OPERATIONCANCELLED Then
                showFormError("Operation was cancelled.")
            Else
                showFormError("Img_OCR :" & res)
                Exit Sub
            End If
    ...
    Private Sub ShowText()
        Try
            ...
            If txt.Trim <> "" Then
                'File.WriteAllText(Application.StartupPath & "\TempRecText.txt", txt)
                Dim TmpInfo() As String = Split(GetPersonInfo(txt.Trim), "##")
                Dim PassportFullName As String = TmpInfo(0) & " " & TmpInfo(1)
                Dim PassportNo As String = TmpInfo(2).Trim

                If PassportFullName.Trim <> "" Then
             
                    frm.fullname = PassportFullName
                    frm.title = "" 'lblDisplayName.Text.Split(" ")(0)
                    If TmpInfo(3).Trim = "THA" Then
                        frm.country = "TH"
                    End If

                  
                    frm.passport = PassportNo
                    frm.DisplayInfo()
                    frm.BringToFront()
            ...
            End If
        Catch ex As Exception
            MessageBox.Show("ไม่สามารถอ่านข้อมูลได้ ลองใหม่อีกครั้ง")
            Me.Close()
        End Try
    ...
    Private Function GetPersonInfo(tmpInfo As String) As String
        Dim ret As String = ""
        If tmpInfo.Trim <> "" Then
            Dim sPos As Integer = tmpInfo.IndexOf("<<")   'ตำแหน่งของเครื่องหมาย << ก่อนถึงชื่อ
            Dim ensSpos As Integer = tmpInfo.IndexOf("<<<<<<")  'ตำแหน่ง Digit สุดท้ายของชื่อ

            Dim FirstName As String = tmpInfo.Substring(sPos + 2, ensSpos - (sPos + 2))
            Dim Lastname As String = tmpInfo.Substring(8, sPos - 8)

            Dim PassportNo As String = ""
            Dim PassportCountry As String = ""
            Dim nxtStr As String = tmpInfo.Substring(ensSpos)   'ข้อมูล Text ที่เหลือหลังจากตัด ชื่อ-สกุล ออกแล้ว
            If nxtStr.Trim.Length > 0 Then
                For Each Str As String In Split(nxtStr, "<<")
                    If Str <> "" Then
                        PassportNo = Str.Trim
                        Exit For
                    End If
                Next
            End If
            ret = FirstName & "##" & Lastname & "##" & PassportNo.Substring(0, 9) & "##" & PassportNo.Substring(10, 3)
        End If
        Return ret
    End Function
    ...
    Private Sub btnSaveZone_Click(sender As Object, e As EventArgs) Handles btnSaveZone.Click
        ...
        Dim FileName As String = Application.StartupPath & "\PassportZone.blk"
        Dim res As Integer = NsOCR.Img_SaveBlocks(ImgObj, FileName)
        If res > TNSOCR.ERROR_FIRST Then
            showFormError("btnSaveZone_Click :" & res)
        End If
    End Sub
    ...
    Private Sub btnLoadZone_Click(sender As Object, e As EventArgs) Handles btnLoadZone.Click
        ...
        If File.Exists(FileName) = True Then
            ...
            If res > TNSOCR.ERROR_FIRST Then
                showFormError("Img_LoadBlocks " & res)
                Exit Sub
            End If
            ShowImage()
        End If
    ...
    Private Sub pbPassportDisplay_MouseDown(sender As Object, e As MouseEventArgs) Handles pbPassportDisplay.MouseDown
        Dim w, h As Integer
    ...
    Private Sub pbPassportDisplay_MouseMove(sender As Object, e As MouseEventArgs) Handles pbPassportDisplay.MouseMove
        GC.Collect()
    ...
    Private Sub pbPassportDisplay_MouseUp(sender As Object, e As MouseEventArgs) Handles pbPassportDisplay.MouseUp
        ...
        If res > TNSOCR.ERROR_FIRST Then
            showFormError("Img_AddBlock : " & res)
            Exit Sub
        End If
